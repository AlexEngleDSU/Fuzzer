...
Connection: keep-alive`
    ...
    list := widget.NewList(
		...
		func() fyne.CanvasObject { 
                // We use the custom compactLink from your original code
                return &compactLink{widget.NewHyperlink("", nil)} 
                },
		func(id widget.ListItemID, obj fyne.CanvasObject) {
			...
			link.SetText(text)
                ...
                // --- RESTORED LINK CLICK LOGIC ---
			if idx := strings.Index(text, "http"); idx != -1 {
				urlPart := text[idx:]
				if spaceIdx := strings.Index(urlPart, " "); spaceIdx != -1 {
					urlPart = urlPart[:spaceIdx]
				}
				if parsed, err := url.Parse(strings.TrimSuffix(urlPart, "/")); err == nil {
					link.URL = parsed
                    			link.Refresh() // Ensure the UI updates the link target
				}
			}
                // ---------------------------------
		},
    ...
    trap := &mouseTrap{onInteract: func() {
        if *followMode {
            *followMode = false
            resumeBtn.Show()
        }
    ...
    trap.ExtendBaseWidget(trap)

    resultsContent := container.NewBorder(nil, resumeBtn, nil, nil, container.NewMax(list, trap))
    ...
    startButton := widget.NewButton("Start Scan", HandleStartScan(
        tabs, list, &resultsList, &mu, urlEntry, userHeaderInput,
