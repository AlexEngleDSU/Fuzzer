...
func StartGUI() {
    ...
    var resultsList = new([]engine.ScanResult)
    var mu sync.Mutex
...
Connection: keep-alive`
	...
	func(id widget.ListItemID, obj fyne.CanvasObject) {
		...
		if res.Location != "" {
			text += fmt.Sprintf(" -> Redirect: %s", res.Location)
		}
		link.SetText(text)
		...
		if err == nil {
			link.URL = parsed
			// This is the critical piece: manually setting the OnTapped function
			link.OnTapped = func() {
				if *followMode {
					*followMode = false
				}
    ...
    resultsContent := container.NewBorder(nil, resumeBtn, nil, nil, list)
