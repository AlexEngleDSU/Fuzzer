...
func ConcurrentScan(ctx context.Context, host, urlTemplate, headerTemplate string, wordlist []string, workerCount int, filterCodes string, recursive bool, maxDepth int, delay time.Duration) <-chan ScanResult {
	...
	go func() {	
		...
		closeChannel := func() {
			closeOnce.Do(func() { close(results) })
		}
			...
			for w := 1; w <= workerCount; w++ {
				...
				go func() {
					...
					for target := range jobs {
						...
						if val, ok := lastRedirectURL.Load("latest"); ok { req.Header.Set("Referer", val.(string)) }
						...
						if !filterMap[status] {
						    if status == 200 && currentLen == badContentLength { continue }
							...
							// Add FUZZ to the path so the loop below will replace it
							newBase := dirPath + "FUZZ"
							mu.Lock()
							nextGen = append(nextGen, newBase)
							mu.Unlock()
						    }
						}
					}
