...
type FuzzerUI struct {
	StatefulRadio    *widget.RadioGroup
	ClearJarCheck    *widget.Check
	CookieAllowList  *widget.Entry
...
func (ui *FuzzerUI) GetOptions() engine.ScanOptions {
    isStateful := false
    if ui.StatefulRadio != nil { isStateful = (ui.StatefulRadio.Selected == "Stateful") }
    timeoutVal, err := strconv.Atoi(ui.TimeoutEntry.Text)
    if err != nil || timeoutVal <= 0 { timeoutVal = 30 }
    
    var allowList []string
    if ui.CookieAllowList.Text != "" {
        raw := strings.Split(ui.CookieAllowList.Text, ",")
        for _, v := range raw {
            allowList = append(allowList, strings.TrimSpace(v))
        }
    }
