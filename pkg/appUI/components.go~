...
func NewFuzzerUI() *FuzzerUI {
	ui := &FuzzerUI{
		// Browser Controls
		StatefulRadio:    widget.NewRadioGroup([]string{"Non-Stateful", "Stateful"}, nil),
		ClearJarCheck:    widget.NewCheck("", nil),
		CookieAllowList:  widget.NewMultiLineEntry(),
		ScanButton:       widget.NewButton("Initiate Scan", nil),
		// Scan Controls
	...
	// Config Defaults
	ui.StatefulRadio.SetSelected("Non-Stateful")
	ui.CookieAllowList.SetPlaceHolder("session_id, user_pref")
	// Scan Defaults
	ui.URLEntry.SetPlaceHolder("https://example.com/FUZZ")
